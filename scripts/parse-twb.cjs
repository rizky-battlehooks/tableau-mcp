...
/*
  ...
  Usage: node scripts/parse-twb.cjs <path-to-twb> [--limit N]
*/
...
function parseWorkbookXml(xmlContent) {
  ...
  try {
    ...
    // swallow and return whatever we collected
  }
...
(function main() {
  ...
  if (!twbArg) {
    console.error('Usage: node scripts/parse-twb.cjs <path-to-twb> [--limit N]');
    process.exit(1);
  }
